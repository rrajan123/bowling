...
# Flask 3-safe: run before every request; create DB if missing
@app.before_request
def ensure_db():
...
def compute_alley_breakdown(db, bowler_id):
    ...
    return [ {"alley": r["alley"], "avg": round(r["avg_score"],2) if r["avg_score"] is not None else 0.0, "games": r["games"]} for r in rows ]
...
def api_lookups():
    ...
    alleys  = rows_to_dicts(db.execute("SELECT id,name FROM alleys ORDER BY name"))
...
def api_save_session():
    ...
    try:
        ...
        # Original behavior: unique(bowler_id, session_date). Insert-or-ignore, then update alley id.
        ...
        # Ensure alley matches latest selection
        db.execute("UPDATE sessions SET alley_id=? WHERE id=?", (alley_id, sid))

        # Replace games for that session
        db.execute("DELETE FROM games WHERE session_id=?", (sid,))
        for i, sc in enumerate(scores, start=1):
...
def api_totals():
    ...
    for b in bowlers:
        ...
        trow = {"name": b["name"], "bix_group": b["bix_group"], **met, "alleys": alleys}
        totals.append(trow)
    ...
    return jsonify({"totals": totals, "bix_diff": bix_diff, "bix_message": bix_msg, "monthly_avgs": by_name})
